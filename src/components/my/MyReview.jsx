        ...
        <tbody>
          ...
          {filteredReviews.map((reviews) => (
            <tr key={reviews._id}>
              ...
              <td className='font-semibold text-lg'>{reviews.game}</td>
              <td className='font-semibold text-lg'>{reviews.genres}</td>
              <td className='font-semibold text-lg'>{reviews.rating} ⭐</td>
              <td className='font-semibold text-lg w-80'>
                {reviews.description}
              </td>
              <td className='font-semibold text-lg'>
                <div className="flex flex-col items-center gap-3">
                <button onClick={()=>handleReviewDelete(reviews._id)} className="bg-gradient-to-r from-orange-400 to-orange-600 text-white font-bold text-2xl px-4 py-3 "><IoTrashBinOutline /></button>

            <Link to={`/myReview/update/${reviews._id}`}>
            <button className="bg-gradient-to-r from-orange-400 to-orange-600 text-white font-bold text-2xl px-4 py-3 "><IoPencil /></button>
            </Link>
